...
async function LoginPage() {
  ...
  try {
    let getData = await fetch(`${API_URL}/auth/login`, { method: "GET" });
    if (getData.ok) {
      let data = await getData.json();
      if (data.success) {
        if (data.data.length === 0) {
          new getStarted(mainContainer);
          return;
        }
        mainContainer.innerHTML = ComponentLogin(data.data);
        CheckLogin();
      }
    } else {
      mainContainer.innerHTML = Modal(
        PpoUp("Erro no servidor: " + getData.statusText, "error")
      );
    ...
    mainContainer.innerHTML = Modal(
      PpoUp(
        "Erro de conexão. Verifique sua internet ou tente novamente mais tarde. erro: " + error,
        "error"
      )
    );
    ...
    if (dataUsername) {
      ...
      try {
        let getData = await fetch(
          `${API_URL}/auth/checkusername/${dataUsername}`,
          { method: "GET" }
        );
        if (getData.ok) {
          CloseLoad();
              ...
              .addEventListener("click", async () => {
                ...
                if (passwordInput.value.trim() == "" || null) {
                  ...
                  try {
                    ...
                    let sendData = await fetch(`${API_URL}/auth/login`, {
                      method: "POST",
                      body: form,
                    });
                    if (sendData.ok) {
...
function setPassword(username) {
  ...
  inputs[1].addEventListener("input", () => {
    if (inputs[0].value == "") {
      ...
      mainContainer.querySelector(".textError").innerHTML =
  ...
  mainContainer.querySelector("#eyeButton").addEventListener("click", () => {
    if (inputs[0].type == "password" && inputs[1].type == "password") {
      ...
      mainContainer.querySelector(
        "#eyeButton"
      ).innerHTML = `<i class="bi bi-eye-slash-fill"></i>`;
    }
  });
  ...
  mainContainer
    ...
    .addEventListener("click", async () => {
      ...
      if (inputs[0].value === inputs[1].value) {
        try {
          const data = { password: inputs[1].value, username: username};
          const sendData = await fetchApi({uri:"/auth/setpassword", data:data, method:"POST"});
          if (sendData.success === true) {
            textError.innerHTML = sendData.message;
            setTimeout(() => {
              LoginPage();
            }, 3000);
            return;
